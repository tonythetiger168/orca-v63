...
#(
  ...
  input  logic        cfg_ecc_inject,     // test: 寫入時污染 ECC, 觸發 error 偵測
  ...
  /* verilator coverage_off */  // 行為模型未使用欄位 (COV_EXCL)
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      cmdq_wr_ptr <= '0;
      cmdq_rd_ptr <= '0;   // bug fix v6.3.4: 原本無出隊, 佇列滿後壅塞
    end else begin
      // 出隊: 命令被排程執行後自佇列移除 (bug fix v6.3.4)
      if (schedule_valid) cmdq_rd_ptr <= cmdq_rd_ptr + 1;
      if (req_valid && req_ready) begin
        // bank IDLE -> ACTIVATE; ACTIVE -> WRITE (req_we) / READ (else)
        // (bug fix v6.3.4: 原本忽略 req_we, WRITE 路徑永不觸發)
        // bank IDLE->ACTIVATE; ACTIVE 且 row miss->PRECHARGE; ACTIVE row hit->WRITE/READ
        // (bug fix v6.3.4: row miss 原本也給 READ/WRITE, 因 row 不符永遠排程不了)
        cmd_queue[cmdq_wr_ptr % CMDQ_DEPTH].cmd_type <=
            (bank_state[addr_stack][addr_channel][addr_bank] == BANK_IDLE) ? 3'd0 :
            (bank_open_row[addr_stack][addr_channel][addr_bank] != addr_row) ? 3'd3 :
            (req_we ? 3'd2 : 3'd1);
  ...
  always_comb begin
    ...
    // Strict-FIFO: 只檢查佇列頭 (cmdq_rd_ptr)
    // (bug fix v6.3.4: 原本 round-robin 亂序排程 + rd_ptr++ 出隊 -> 移除錯誤 slot,
    //  佇列狀態腐化卡死。改 strict-FIFO 後出隊與排程一致, 有 head-of-line blocking 但正確)
    if (!cmdq_empty) begin
      automatic int idx = cmdq_rd_ptr % CMDQ_DEPTH;
      automatic cmd_t cmd = cmd_queue[idx];
      automatic bank_state_t bstate = bank_state[cmd.stack][cmd.channel][cmd.bank];
  ...
  // ---------------------------------------------------------------------------
  // 讀資料路徑 (bug fix v6.3.4: rsp_data/read_ecc 原本未驅動 -> 讀回無資料、ECC 檢查恆過)
  // 行為模型: 每 stack 儲存最後寫入的 {data, ecc}, 讀回該 stack 資料供 ECC 檢查
  // ---------------------------------------------------------------------------
  logic [DATA_WIDTH-1:0]   mem_data [NUM_STACKS];
  logic [DATA_WIDTH/8-1:0] mem_ecc  [NUM_STACKS];
  logic [3:0]              rsp_stack_q;
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      for (int st = 0; st < NUM_STACKS; st++) begin
        mem_data[st] <= '0; mem_ecc[st] <= '0;
      end
      rsp_stack_q <= '0;
    end else begin
      if (schedule_valid && scheduled_cmd.cmd_type == 3'd2) begin   // WRITE: 存資料+ECC
        mem_data[scheduled_cmd.stack] <= scheduled_cmd.data;
        // 逐 64-bit word 計算 ECC (bug fix: 原本 calculate_ecc(512bit) 只算低 64bit,
        //  mem_ecc 高位元組全 0 -> 讀回 word1-7 ECC mismatch 誤報)
        for (int i = 0; i < DATA_WIDTH/64; i++) begin
          mem_ecc[scheduled_cmd.stack][i*8 +: 8] <= calculate_ecc(scheduled_cmd.data[i*64 +: 64])
                                                    ^ {7'b0, cfg_ecc_inject};  // 測試注入
        end
      end
      if (schedule_valid && scheduled_cmd.cmd_type == 3'd1)         // READ: 鎖定 stack
        rsp_stack_q <= scheduled_cmd.stack;
    end
  end
  assign rsp_data = mem_data[rsp_stack_q];
  assign read_ecc = mem_ecc[rsp_stack_q];

  // rsp_valid: READ 排程完成下一拍拉起 (bug fix v6.3.4: rsp_valid 原本未驅動 -> 讀完成握手永不觸發)
  logic rsp_valid_q;
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) rsp_valid_q <= 1'b0;
    else        rsp_valid_q <= schedule_valid && (scheduled_cmd.cmd_type == 3'd1);
  end
  ...
  // PHY 重設/RWDS (bug fix v6.3.4: 原本未驅動 -> 懸浮輸出)
  // phy_dqs_t/c 為 inout 資料選通, 由 PHY(非控制器邏輯)驅動, 控制器不驅動 (架構正確)
  always_ff @(posedge clk_phy or negedge rst_n) begin
    if (!rst_n) begin
      phy_rst_n <= {NUM_STACKS{1'b0}};
      phy_rwds  <= {NUM_STACKS{1'b0}};
    end else begin
      phy_rst_n <= {NUM_STACKS{1'b1}};   // 重設釋放
      phy_rwds  <= {NUM_STACKS{schedule_valid &&
                     (scheduled_cmd.cmd_type == 3'd1 || scheduled_cmd.cmd_type == 3'd2)}};
    end
  end
