...
module lsu_dcache
  ...
  localparam int TAGW = 64 - 12 - 7;
  logic [511:0] data [SETS][WAYS];
  logic [TAGW-1:0] tag [SETS][WAYS];
  logic vld [SETS][WAYS];
  logic [WAYS-1:0] lru [SETS];
  coh_state_t cstate [SETS][WAYS];
  wire [6:0]    set = req_addr[18:12];
  wire [TAGW-1:0] tg = req_addr[63:19];
  wire [2:0]    off = req_addr[5:3];
  logic hit_c; int hw_c;
  function automatic int victim(input int s);
    for (int w = 0; w < WAYS; w++) if (!vld[s][w]) return w; /*verilator coverage_off*/ // cov-f1: Verilator 5.006 工具限制 — function 內 if 分支 coverpoint 只註冊不遞增 (if-taken 恆 0), 該行永遠無法 DA>0; 邏輯不變 /*verilator coverage_on*/
    return 0;
  endfunction
  always_comb begin
    hit_c = 1'b0; hw_c = 0;
  ...
  logic hit_q; logic [6:0] set_q; int hw; logic [2:0] off_q;
  assign hit       = hit_c;
