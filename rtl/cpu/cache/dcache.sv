...
// dcache: 64KB 8-way 64B-line behavioral cache (single clock)
module dcache
  import orca_pkg::*; (
  ...
  input  logic      fill_valid, /* verilator coverage_off */ // cov: tie-off 恆 0, 無 driver 可翻動 (fill_dirty 由 lsu_dcache 綁 1'b0)
  input  logic      fill_dirty, /* verilator coverage_on */
  ...
  coh_state_t cstate [SETS][WAYS]; /* verilator coverage_off */ // cov: wire 宣告初始化被工具別名化, 無獨立賦值點 (tool limit)
  wire [SW-1:0]  set = req_addr[SW+11:12]; /* verilator coverage_on */
  ...
  logic hit_q; logic [SW-1:0] set_q; int hw; logic [2:0] off_q; /* verilator coverage_on */
  assign hit       = hit_c;
