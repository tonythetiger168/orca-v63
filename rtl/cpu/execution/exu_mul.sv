...
module exu_mul
  ...
  // bug fix v6.3.4: R-type (M-ext) 的 imm=i_imm, imm[14:12] 非 funct3。
  // 原本讀 imm[14:12] 恆為 0 -> 全部誤判 MUL, DIV/REM 永不進除法器。
  assign funct3 = uop.funct3;
  ...
  logic div_by_zero;
  // bug fix v6.3.4: 用鎖存 div_divisor (接受時捕捉 abs_b) + 除法器活動閘控。
  // 原本用即時 is_div -> DIV_DONE 時若 exu_mul 閒置則 is_div=0, 除零例外永不觸發。
  assign div_by_zero = (div_divisor == 64'b0) && (div_state != DIV_IDLE);
