//============================================================================
// ORCA v6.2 ZEN — Phoenix+ TPE (Multi-Chip Tensor Engine)
// Peak Performance: 16 TOPS per die @ 3.2GHz (INT8), 32 TOPS dual-die
// Target Process: 5nm
// Area Estimate: ~8.5 mm2 per die
// Power Estimate: <15W per die
//
// Features:
// - Same 8 tiles x 16x16 as Phoenix v6.0 per die
// - UCIe/Chiplet die-to-die interface for multi-die scaling
// - Die-to-die coherent link (256-bit, 16 GT/s)
// - Global address space across 2 dies
// - Distributed tile scheduling
//============================================================================
`ifndef ORCA_TPE_V6_2_SV
`define ORCA_TPE_V6_2_SV
`include "orca_params.sv"
`include "orca_pkg.sv"
module orca_tpe_v6_2 #
...
// --------------------------------------------------------------------------
// AXI4-Stream / CHI Interface to memory (512-bit)
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// UCIe Die-to-Die Interface (to other Phoenix+ die)
// --------------------------------------------------------------------------
...
// Link training: exchange sync patterns
...
// ==========================================================================
//  Command Decoder with Die-Aware Routing
// ==========================================================================
...
, TPE_D2D_SYNC    = 4'd5
// Multi-die synchronization
, TPE_ACTIVATION  = 4'd6
, TPE_STORE       = 4'd7
, TPE_DONE        = 4'd8  } tpe_state_t;
  tpe_state_t state, next_state;
  logic [7:0]  cmd_op;
  logic        cmd_target_die;
// 0=this die, 1=remote die
assign cmd_op = cmd_data[7:0];
assign cmd_target_die = cmd_data[40];
 // Die select bit
// ==========================================================================
//  Tile SRAM Arrays (8 tiles per die)
// ==========================================================================
...
// ==========================================================================
//  8 x 16x16 Systolic Tile Array (same as v6.0)
// ==========================================================================
...
case (state)
TPE_IDLE:
if (cmd_valid && !cmd_target_die)  next_state = TPE_CMD_DECODE;
TPE_IDLE:
if (cmd_valid && cmd_target_die)   next_state = TPE_D2D_SYNC;
...
TPE_D2D_SYNC:     next_state = TPE_CMD_DECODE;
...
assign mem_araddr  = cmd_data[63:0];
assign mem_rready  = 1'b1;
assign mem_awvalid = (state == TPE_STORE);
assign mem_awaddr  = cmd_data[63:0] + 64'h10000;
